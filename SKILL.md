...
description: "诊断 Windows、macOS 或 Linux 电脑及平台服务出现的严重卡顿、无响应、频繁转圈、输入延迟、磁盘忙、内存不足或整体变慢问题。用户提到系统很卡、电脑卡死、Mac 卡顿、Linux 服务卡顿、平台卡顿、CPU/内存/磁盘占用异常、程序无响应、想排查整机性能、找出哪个进程拖慢电脑、分析近期系统错误时，即使没有明确要求运行诊断，也应使用本技能。"
compatibility: "Windows PowerShell 5.1+（CIM/WMI）、macOS 自带 Bash 3.2+，或 Linux Bash 3.2+（/proc、ps、df、systemctl/journalctl）；默认只读。"
---
...
Linux 使用 Bash 脚本，从 `/proc` 和常见系统工具采集整体负载、内存、磁盘、网络、服务和日志：
...
| 类别 | 关注信号 | 高优先级线索 |
|------|----------|--------------|
| CPU | `cpu.percentProcessorTime`、Windows 的 `systemLoad.processorQueueLength`、Linux 的 `systemLoad.load1`/`processorQueueLength`、`topProcessesByCpu` | 总 CPU 连续约 85% 以上；Windows 队列持续大于逻辑核心数，或 Linux 运行队列和 load average 持续偏高 |
| 内存 | Windows 的 `memory.percentCommittedBytesInUse`；macOS 的 `availableMiB`、`compressedMiB`；Linux 的 `availableMiB`、`percentUsed`、`swapUsedMiB` | Windows 已提交内存约 85% 以上；macOS 或 Linux 可用内存很低且压缩/交换空间持续增长 |
| 磁盘 | Windows 的延迟/队列；Linux 的 `disks.readBytesPerSec`、`writeBytesPerSec`、`ioInProgress`；所有系统的卷可用空间 | 任一系统盘剩余空间低于约 10% 都应标注。macOS 脚本不推断磁盘延迟。 |
...
| 系统 | `recentEvents`、`services`、`uptimeHours` | Windows 的磁盘/驱动错误、macOS 统一日志，或 Linux journal/kernel 事件与卡顿时间吻合 |

对于正在运行的性能诊断、杀毒扫描、IDE 编译、虚拟机、容器、同步客户端、Windows Update、macOS 系统更新/Spotlight 索引，或 Linux 包更新、日志轮转、备份任务，要明确说明它们可能是短暂负载，先确认是否与用户操作一致。不要仅凭进程名断言恶意或故障。
...
- 出现磁盘 I/O 错误、控制器重置、持续蓝屏、kernel panic、Linux OOM/内核错误、硬件温度/风扇异常或疑似数据损坏时，优先提醒用户备份重要数据；涉及磁盘检测、驱动或系统变更、服务重启、硬件维修的动作先征求批准。
